...
form.addEventListener('submit', (e)=>{
      ...
      if (data.success) {
          ...
          setTimeout(() => {
              ...
              success.innerHTML = `
                ...
                <h2 style="margin-bottom:8px;color:var(--primary);font-weight:700">You’re All Set!</h2>
                <p style="margin-bottom:18px;line-height:1.6">
                  Thanks for Choosing the Road with Us. Our travel experts will be in touch shortly with the next steps.
                </p>
                <button 
          ...
          if (data.errors) {
              Object.keys(data.errors).forEach(field => {
                  ...
                  if (input) {
                      ...
                      errorElem.innerHTML = data.errors[field].join('<br>');
                      input.parentNode.appendChild(errorElem);
                  }
              });
          }

          // Optional: also show general flash message
          // showFlashMessage(data.message || "Something went wrong. Please try again.", 'error');
  ...
  .catch(error => {
      // showFlashMessage("1. Submission failed. Please try again.", 'error');
      submitBtn.innerHTML = 'Submit Application';
      submitBtn.disabled = false;
  });
...
function showFlashMessage(message, type = 'success') {
  ...
  // color by type
  if (type === 'success') flash.style.background = '#4CAF50';
  if (type === 'error') flash.style.background = '#E53935';
  if (type === 'warning') flash.style.background = '#FFC107';
