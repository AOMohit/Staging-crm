                        ...
                        <div class="dt-buttons btn-group flex-wrap">
                            ...
                            <a class="btn btn-secondary btn-primary text-white" href="{{ route('agent.add') }}"
                                tabindex="0"><span><i class="mdi mdi-plus me-sm-1"></i>
                                    <span class="d-none d-sm-inline-block">Add Agent</span></span>
                            </a>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal fade" id="basicModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('agent.import') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col mb-4 mt-2">
                                ...
                                <a href="{{ route('agent.export-sample') }}">click here</a> to download sample file
                            </div>
                        </div>
            ...
            var table = $('#myDatatable').DataTable({
                ...
                "columns": [{
                        ...
                        "render": function(data, type, row, meta) {
                            var id = row.id;
                            var routeEdit = "{{ route('agent.edit', ['id' => 'rowID']) }}";
                            routeEdit = routeEdit.replace('rowID', id);

                            var routeView = "{{ route('agent.view', ['id' => 'rowID']) }}";
                            routeView = routeView.replace('rowID', id);

                            var routeDlt = "{{ route('agent.delete', ['id' => 'rowID']) }}";
                            routeDlt = routeDlt.replace('rowID', id);

                            var routeActivity = "{{ route('agent.activity', ['id' => 'rowID']) }}";
                    ...
                    },
                    {
                        ...
                        "render": function(data, type, row, meta) {
                            ...
                            var text =
                                `<a class="dropdown-item waves-effect text-primary" href="${routeView}">${row.agency}</a>`;
                            return text;
                        }
                    }, {
                        name: 'agent',
                        "render": function(data, type, row, meta) {
                            var text =
                                `<div><div>${row.name} <div> <div>${row.email}</div></div>`;
                            return text;
                        }
                    }, {
                        data: 'phone',
