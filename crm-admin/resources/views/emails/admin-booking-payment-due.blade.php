            ...
            <tbody>
                <tr>
                    <td style="font-size: 15px; line-height: 24px; font-family: 'Helvetica', Arial, sans-serif; font-weight: 400; text-decoration: none; color: #383838; width: 596px; vertical-align: top; padding-left: 30px; padding-right: 30px; padding-top: 30px; padding-bottom: 40px;"
                        ...
                        <p>Hi Team,</p>
                        <p>This email serves as a reminder that a payment for an upcoming trip is due.</p>
                        <p>Customer: {{ $data['customer_name'] }}</p>
                        <p>Trip: {{ $data['trip_name'] }}</p>
                        <p>Trip Date: {{ $data['trip_date'] }}</p>
                        <p>Due Date: {{ $data['due_date'] }}</p>
                        <p>Amount Due: ₹{{ $data['pending_amt'] }}</p>
                        <br>
                        <p>------------------------</p>
                        <p>Total Receivables: ₹{{ $data['total_recievables'] }}</p>
                        <p>Total Received: ₹{{ $data['total_rec'] }}</p>
                        <p>Net Receivables: ₹{{ $data['net_recievables'] }}</p>
                    </td>
                </tr>
            </tbody>
        </table>
          ...
          <tr>
            ...
            <img src="{{ asset('storage/app/admin/setting/AO-footer-mailer.png') }}" style="width:auto;height:160px">
              <p style="font-size: 12px; line-height: 15px; font-family: 'Helvetica', Arial, sans-serif; font-weight: normal; text-decoration: none; color: #000000; margin-top: 5px;">
                This email was sent to {{ $data['admin_email'] }}.<br>
