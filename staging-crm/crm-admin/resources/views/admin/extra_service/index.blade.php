                ...
                <div class="p-3">
                    <div class="dt-action-buttons text-end pt-3 pt-md-0">
                        <div class="dt-buttons btn-group flex-wrap">
                            ...
                            <a class="btn btn-secondary btn-primary text-white"
                                href="{{ route('setting.extra_service.add') }}" tabindex="0"><span><i
                                        class="mdi mdi-plus me-sm-1"></i>
                                    <span class="d-none d-sm-inline-block">Add Extra Service</span></span>
                            </a>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        function getfilterdata() {
            ...
            var table = $('#myDatatable').DataTable({
                ...
                "destroy": true,
                "ajax": {
                    "url": "{!! route('setting.extra_service.get') !!}",
                    "type": 'GET',
                    "data": {
                        "user_type": "admin",
                    }
                ...
                "columns": [{
                        ...
                        "render": function(data, type, row, meta) {
                            var id = row.id;
                            var routeEdit = "{{ route('setting.extra_service.edit', ['id' => 'rowID']) }}";
                            routeEdit = routeEdit.replace('rowID', id);

                            var routeDlt = "{{ route('setting.extra_service.delete', ['id' => 'rowID']) }}";
                    ...
                    },
                    {
                        ...
                        "render": function(data, type, row, meta) {
                            var text = '';
                            if (row.is_redeemable == 1) {
                                text = `<span class="badge bg-success">Yes<span>`;
                            } else {
                                text = `<span class="badge bg-danger">No<span>`;
                            }
                            return text;
                        }
                    },
