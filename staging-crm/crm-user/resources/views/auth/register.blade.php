<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div>
            ...
            <x-text-input id="name" class="block mt-1 w-full" type="text" name="phone" :value="old('name')" required autofocus autocomplete="name" />
            <x-input-error :messages="$errors->get('phone')" class="mt-2" />
        </div>

        <div>
            <x-input-label for="name" :value="__('Gender')" />
            <input type="radio" name="gender" id=""><span class="ps-1">Male</span>
            <input type="radio" name="gender" id=""><span class="ps-1">Famale</span>
        </div>
